...
namespace FluUrl.Repository
{
    public class RedisOperationRepository: IRedisOperationRepository {
        ...
        /// <param name="redisValue">元素</param>
        /// <param name="score">分数</param>
        public async Task<IEnumerable<string>> SortedSetRangeByRankAsync(string redisKey, int start=0, int stop=1)
        {
            var result=await _database.SortedSetRangeByRankAsync(redisKey, start, stop);
            return result.Select(o => o.ToString());
        }

        public Task SortedSetAddAsync(string key, string v, object p)
        {
